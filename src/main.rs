...
///    ```
///
const MIN_CONFIG_FILE_VERSION: Option<&'static str> = Some("1.7.1");
/// (c) Jens Getreu
...
fn create_new_note_or_synchronize_filename(path: PathBuf) -> Result<PathBuf, anyhow::Error> {
    ...
    if path.is_dir() {
        let (n, new_fqfn) =
            if matches!(*STDIN, Content::Empty) && matches!(*CLIPBOARD, Content::Empty) {
                ...
                (n, new_fqfn)
            } else if matches!(*STDIN, Content::HeaderAndBody{..})
