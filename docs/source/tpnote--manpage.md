...
note's metadata with its filename. Tp-Note analyzes its environment and the
...
After the user finishes editing, Tp-Note analyzes potential changes in the
note's metadata and renames, if necessary, the file so that its metadata and
...
template extracts the first line of the clipboard's content and inserts it into
...
Use case: Open a web page in your web browser and select some text with its
...
create the following document:
    ...
    ./Fairy tales/20250104-Cinderella--Notes.md
...
etc. in the HTML clipboard and uses the finding as the title for the new
Tp-Note document.
...
content template uses the link text of the first hyperlink found as the document
title.
...
Furthermore, this operation mode is very handy with pipes in general, as shown
by the following example: it downloads some webpage, converts it to Markdown and
...
The configuration file variable '`filename.extensions`' lists all the file
...
_all your note's filenames always correspond to their metadata_, which helps
...
> Loads an additional configuration from the TOML formatted _FILE_
> and merges it into the default configuration.

**-C** _FILE_, **\--config-defaults**=_FILE_

> Dumps the internal default configuration in TOML format into _FILE_ or
> stdout if _FILE_ equals to '`-`', e.g. '`tpnote -C - | less`'.

**-d** _LEVEL_, **\--debug**=_LEVEL_

> Prints additional log messages. The debug level _LEVEL_ must be one out of
...
> '`-n -d info --edit`'. Or, to observe the launch of the web browser: 
> '`-n  -d info --view`'. The option '`-d trace`' shows all available template
> variables, the templates used and the rendered result of the substitution.
> This is particularly useful for debugging new templates. The option 
> '`-d  off`' silences all error message reporting and also suppresses the error
> pop-up windows.
...
> value _PORT_. If not given, a random available port is chosen automatically.
...
**-s** _SCHEME_NAME_, **\--scheme**=_SCHEME_NAME_
...
**-x** _DIRECTORY_, **\--export**=_DIRECTORY_
...
document structure as shown in the figure below. In
this documentation
the terms "YAML header", "header" and "front matter" are used as
synonyms to designate the document's metadata block at the beginning
of the text file:
...
A so-called _sort tag_ is an alphanumeric prefix at the beginning of the
...
- _Sequence number sort tag_
  ...
  criterion to recognize a sequence number sort-tag. The largest counter in
  these examples is '`144`', so all sort_tags are sequence numbers.

- _Alphanumeric sequence number sort tag_
...
2. A counter is a set of digits (base 10) '`123`' or a set of lowercase
   letters (base 26) '`ab`'.

3. A letter-based counter can be maximal 2 letters wide. Its maximum is
   '`zz`' (cf. '`filename.sort_tag.letters_in_succession_max`').

4. A _sequential sort-tag_ is a sort-tag whose counters are at most 3
...
> filename on disk without _sort tag_ == '`{{ fm.fm_title }}--{{ fm.fm_subtitle }}.md`'
...
quotes in order to label it as a string type. When Tp-Note synchronizes the
...
The template shows all variables with their default values. When you change
...
and their command line arguments for Unix-like operating systems.
For other systems consult: '`app_args.windows.editor`',
...
to be considered. '`words_total_percentage_min`' sets the minimum word count
...
is useful, because the detection algorithm cannot figure out the region code
...
By default, the front matter variable names are printed in English when creating
...
default internal '`tmpl.annotate_file_content`' template handles "Markdown" and
...
characters '`_`', '`-`', '`.`' are recognized as being part of a _sort tag_
...
in filenames, which avoids potential cyclic filename changes. Or, in other
...
<table class="fm">
  <tr>
    ...
    <th class="fmval">
      <b> {{ fm.fm_title| default(value='') | to_html | safe }}</b>
    </th>
  </tr>
  <tr>
  ...
  {% for k, v in fm | remove(key='fm_title')| remove(key='fm_subtitle')| %}
  <tr>
...
control over the CSS input coming from the configuration file variables
...
discussed in the chapters: _Customize the built-in note viewer_ and _Choose your
...
- '`{{ html_clipboard.body | link_title }}`' is the title of the first Markdown
  or ReStructuredText formatted link in the clipboard.
...
- '`{{ doc.body | get_lang }}`' determines the natural
  ...
  configured language candidates, the filter '`{{ get_lang }}`' returns an empty
  array.
...
The '`sanit()`' filter transforms a string in a file system friendly form.
...
6. when the connecting web browser process belongs to your own OS user
   (cf. '`viewer.same_user_policy`' below).

The HTTP server runs as long as the launched web browser window is open. It
exposes not only the displayed note file, but also all referenced inline images
and other linked Tp-Note files; internally the viewer keeps a list of
_referenced local URLs_ and serves only those. Each incoming connection passes,
in order, through the following checks:

1. **Host header.** A request whose HTTP '`Host`' header does not address
   '`localhost`' is refused with '`403 Forbidden`', which defeats DNS rebinding
   attacks.
...
2. **OS user** ('`viewer.same_user_policy`'). While the session is still being
   ...
   before your web browser binds the session.

3. **Session cookie** ('`viewer.session_binding_cookie`'). The viewer binds
   itself to the first web browser that loads the note page. By default that
   browser receives a random session cookie ('`Set-Cookie: tpnote=…; HttpOnly;
   SameSite=Lax`'); from then on every request that does not present the cookie
   is refused with '`403 Forbidden`'. The '`SameSite=Lax`' attribute makes the
   browser withhold the cookie from cross-site background requests such as
   '`fetch()`', '`<img>`' or '`<iframe>`', so a hostile web page open in the
   same browser cannot read the note.
...
stays on, so another local user could read your note only by winning the race
to claim the session in the brief start-up window before your browser binds it -
and even then you would notice, because your own browser would be locked out and
shown a '`403 Forbidden`' page instead of the note. Likewise, if your browser
refuses cookies for '`localhost`', allow them for '`http://localhost`' or
disable the binding with '`viewer.session_binding_cookie = false`'.

**Summary**: Tp-Note is safe to use in a multi-user environment. Its viewer
binds to the '`localhost`' interface only, so it is never reachable from the
